...
function openUpsellItemEditPanel(prod, sizeSelect, pendingCustomRef) {
    ...
    overlay.innerHTML = `
        <div class="upsell-edit-panel" role="dialog" aria-modal="true">
            ...
            <div class="upsell-edit-price-summary">
                ...
                <span class="upsell-edit-price-total" id="ue-total">€${calcUpsellEditPrice(basePrice, { size: currentSize, ...current, patchExtraPrice: current.patchExtraPrice }, isEspana26).toFixed(2)}</span>
            </div>
    ...
    function updatePrice() {
        ...
        el.textContent = `€${calcUpsellEditPrice(basePrice, { size: getSize(), version: getVersion(), name: getName().trim(), number: getNumber().trim(), patch: pState.patch, patchExtraPrice: pState.patchExtraPrice }, isEspana26).toFixed(2)}`;
    }
